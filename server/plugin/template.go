...
// according to the error messages returned in https://forgejo.com/join, it must:
...
func init() {
	...
	funcMap["trimBody"] = func(body string) string {
		if strings.Contains(body, "system@code.syn.st") {
...
{{else}}{{end}}`))
	...
	template.Must(masterTemplate.New("helpText").Parse("" +
		...
		"    * `--include-only-org-members` - events triggered only by organization members will be delivered (the Forgejo organization config should be set, otherwise this flag has not effect)\n" +
		...
		"* `/forgejo setup` - Setup your Forgejo plugin\n" +
...
func lookupMattermostUsername(githubUsername string) string {
	...
	return forgejoToUsernameMappingCallback(githubUsername)
}
